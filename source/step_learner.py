...
def collect_long_step_start_state(env):
    ...
    targets = [0.3, 0.6, 1.4]
    for t in targets:
        end_state, _ = env.simulate([t,0,0], action=action, render=1, put_dots=True)
    env.clear_skeletons()
    return end_state

def learn_long_step(env):
    start_state = collect_long_step_start_state(env)
    runner = Runner(env, start_state, [1.7, 0.0, 0.0])
...
def collect_stair_start_state(env):
    action = np.zeros(SIMBICON_ACTION_SIZE)
    env.reset(random=0.0)
    targets = np.array([[0, 0, 0], [0.4, 0, 0], [0.8, 0, 0], [1.2, 0.1, 0]])
...
def learn_stair(env):
    ...
    runner = Runner(env, start_state, [1.2, 0.1, 0.0])
    rs = RandomSearch(env, runner, 4, 0.1, 0.05)
    rs.random_search()
