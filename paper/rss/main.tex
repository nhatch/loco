...
with weights determined by the change in total reward as defined in section \ref{sec:expert}.
...
Furthermore, we decided that all $2Q$ rollouts during each iteration of ARS should use the same random seed (i.e. the same placement of stepping stones).
...
Results comparing its performance to CIM on 2D-Easy are shown in Fig. \ref{fig:ars baseline}.
The X axis shows the total number of simulated footsteps taken during training, and the Y axis shows total reward.
We plot the median and 5th/95th percentiles over 32 testing episodes (eight episodes for each of the four starting random seeds).
This ARS baseline learned very slowly even for the 2D-Easy environment, so we did not attempt to evaluate it on 2D-hard.
...
Results are shown in Fig. \ref{fig:nocur baseline}.
